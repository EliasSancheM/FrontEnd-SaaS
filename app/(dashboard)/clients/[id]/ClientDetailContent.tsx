...
import { useClient, useUpdateClient, useDeleteClient } from '@/lib/clients';
...
export default function ClientDetailContent({ params }: ClientDetailPageProps) {
  ...
  const paymentRate = clientInvoices.length > 0 ? Math.round((paidInvoicesCount / clientInvoices.length) * 100) : 100;

  const handleEditSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    updateClient.mutate(
      { id: client.id, input: formData },
      {
        onSuccess: () => {
          setIsEditing(false);
          toast.success('Información del cliente actualizada con éxito', toastStyle);
        },
  ...
  const handleDeleteClient = () => {
    if (!confirm(`¿Estás seguro de que deseas eliminar a ${client.name}? Esta acción no se puede deshacer.`)) return;
    deleteClient.mutate(client.id, {
      onSuccess: () => {
        toast.success('Cliente eliminado correctamente', toastStyle);
        router.push('/clients');
      },
      onError: (error) => {
  ...
  const handleMarkAsPaid = (invoiceId: number, invoiceNumber: string) => {
    markPaid.mutate(invoiceId, {
      onSuccess: () => toast.success(`Factura ${invoiceNumber} registrada como PAGADA`, { icon: '💰', ...toastStyle }),
      onError: () => toast.error('No se pudo actualizar la factura', toastStyle),
    });
  };

  const handleDownloadPdf = async (invoiceId: number, invoiceNumber: string) => {
    const t = toast.loading(`Generando PDF de ${invoiceNumber}...`, toastStyle);
    try {
      await downloadInvoicePdf(invoiceId, invoiceNumber);
      toast.success(`PDF de ${invoiceNumber} descargado`, { id: t, ...toastStyle });
    } catch {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">{client.name}</h1>
      ...
      <div className="grid gap-6 sm:grid-cols-4">
        <div className="bg-white dark:bg-[#0e0e0e] border border-zinc-100 dark:border-zinc-900 rounded-2xl p-6 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h4 className="text-lg font-bold text-zinc-900 dark:text-white mt-0.5">{money(totalBilled)}</h4>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0e0e0e] border border-zinc-100 dark:border-zinc-900 rounded-2xl p-6 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h4 className="text-lg font-bold text-zinc-900 dark:text-white mt-0.5">{money(pendingBalance)}</h4>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0e0e0e] border border-zinc-100 dark:border-zinc-900 rounded-2xl p-6 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h4 className="text-lg font-bold text-zinc-900 dark:text-white mt-0.5">{money(totalPaid)}</h4>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0e0e0e] border border-zinc-100 dark:border-zinc-900 rounded-2xl p-6 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h4 className="text-lg font-bold text-zinc-900 dark:text-white mt-0.5">{paymentRate}%</h4>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-[#0e0e0e] border border-zinc-100 dark:border-zinc-900 rounded-2xl p-6 shadow-sm h-fit space-y-6">
          ...
          <div className="space-y-4 text-sm">
            ...
            <div>
              ...
              <span className="font-mono text-zinc-850 dark:text-zinc-300 mt-1 block">{client.rut || '—'}</span>
            </div>
                ...
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900 text-sm">
                  {clientInvoices.map((invoice) => (
                    <tr key={invoice.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/10 transition-colors">
                      ...
                      <td className="py-3.5 px-6 text-zinc-500 dark:text-zinc-450">{invoice.issue_date}</td>
                      <td className="py-3.5 px-6 font-bold text-zinc-900 dark:text-white">{money(invoice.total)}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-lg backdrop-blur-2xl bg-zinc-900/90 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 text-white">
            <div className="flex items-center justify-between p-6 border-b border-zinc-800">
              ...
              <button onClick={() => setIsEditing(false)} className="p-1.5 rounded-lg hover:bg-zinc-800 text-zinc-450 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-zinc-800">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-bold rounded-xl text-sm transition-all cursor-pointer shadow-lg shadow-emerald-500/10 disabled:opacity-60 inline-flex items-center gap-2"
                >
